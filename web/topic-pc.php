...
<div class="modal fade" id="send_mail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">不適切な内容の報告</div>
            <div class="modal-body">
                <p>不適切な箇所を明記の上、送信ボタンを押してください。</p>
                <div class="form-group">
    <form action="/infomsgs/add" id="sendmail_form" method="post" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="POST"/></div>    <input type="hidden" name="data[Infomsg][article_id]" value="87096" id="InfomsgArticleId"/>    <div class="input textarea"><textarea name="data[Infomsg][text]" class="form-control mb10" placeholder="内容" rows="6" id="sendmail_text_area" cols="30"></textarea></div>    <div class="input email"><input name="data[Infomsg][email]" class="form-control mb10" placeholder="メールアドレス" maxlength="255" type="email" id="InfomsgEmail"/></div>    <div class="submit"><input  class="btn btn-default" onclick="sendmail()" type="submit" value="送信する"/></div>    </form>    <div id="sendmail_form_msg"></div>
</div>


<script>
    function sendmail() {
        event.preventDefault(); //stop html submit

        if( $('#sendmail_text_area').val() != '' ){
            if (confirm("入力いただいた内容で送信します。\nよろしいですか？")) {

                var $form = $('#sendmail_form');
                ...
                $.ajax({
                    ...
                    crossDomain: true,
                    error: function() {
                        $("#sendmail_form_msg").html('<p class="gray mt10">送信できませんでした。</p>');
                        setTimeout(function() {
                            $("#sendmail_form_msg").remove();
                        }, 5000);
                    },
                    success: function(data, dataType) {
                        $("#sendmail_form_msg").html('<p class="gray mt10">送信が完了しました。どうもありがとうございました。</p>');
                        setTimeout(function() {
                            $("#sendmail_form_msg").remove();
                        }, 8000);
                    }
            ...
            $("#sendmail_form_msg").html('<p class="red mt10">内容を入力してから、送信ボタンを押してください。</p>');
        }
    }
</script>
            </div>
